<template>
  <div class="wallpaper-browser">
    ...
    <el-tooltip content="下载路径配置" placement="top">
      <el-backtop :bottom="100" :right="0" :visibility-height="0" class="config-button">
        <template #default>
          <el-icon>
            <el-button type="info" :icon="SetUp" circle @click="open_config"/>
          </el-icon>
        </template>
      </el-backtop>
    </el-tooltip>
...
function invokeAPI(apiMethod: string, newPage: number) {
  ...
  invoke(apiMethod, {page: newPage}).then((res: any) => {
    console.log(res);
    res.forEach((fit: any) => {
      fits.push(fit);
    });
